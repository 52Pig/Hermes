...
class RTRMAV3(BaseStrategy):
    ...
    def should_buy(self, stock_code, current_price, last_1d_close_price, info_dict, is_high_fx, is_noon_time):
        # 下午14:00
        cur_time = datetime.datetime.now().time()
        if cur_time < datetime.time(9, 42):
            return False, 198
    ...
    async def do(self, accounts):
        print("[DEBUG]do realtime rma v3", utils.get_current_time(), accounts)
        target_code = '沪深A股'
        ...
        for rank, (code, score) in enumerate(eff_code_dict.items(), 1):
            ...
            is_buy, buy_id= self.should_buy(stock, latest_price, last_1d_close_price, info_dict, is_high_fx, is_noon_time)
