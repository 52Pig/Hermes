...
pp = "D:/tool/dataset/logs"  # 修正这个路径的拼写错误

# 检查并创建日志目录
if not os.path.exists(pp):
    os.makedirs(pp, exist_ok=True)

log_file = os.path.join(pp, f"reverse_moving_average_bull_track_v5_{current_date}.log")
print(log_file)

# 创建一个新的日志记录器
rma_logger = logging.getLogger("ReverseMABULLV5")
...
class ReverseMABULLV5(BaseStrategy):
    ...
    def __init__(self, config):
        ...
        pools_file = self.get_latest_file('D:/tool/dataset/strategy_data', 'opt_rtrma_')
    ...
    def should_sell(self, stock, last_1d_close_price, max_price, max_price_timestamp, last_price, bid_price, bid_vol, has_open_price, open_price):
        """判断是否满足卖出条件"""
        if self.is_today_bought(stock):
            return False, -100  # 特殊代码表示T+1限制
        # 计算持仓天数
        holding_days = self.get_holding_days(stock)

        # 卖：下午14:50以后价格<开盘价
        cur_time = datetime.datetime.now().time()
        # if cur_time > datetime.time(9, 41) and open_price > last_price:
        #     return True, -3
        ## 卖出情况0：止盈位,一天内利润8%
        ...
        if has_open_price > 0:
            ...
            # 持仓超过3天且收益低于6%
            if cur_time > datetime.time(14, 45) and holding_days >= 5 and current_return < 0.049:
                return True, 10

            ## 止损，多天总共亏损成本价的x%
            if (last_price - has_open_price) / has_open_price < -0.078:
                return True, 7
        ...
        if "" != last_close_str and len(last_close_list) > 32:
            ...
            last3d_ma5 = sum(lc_list[:-2][len(lc_list[:-2]) - 5:]) / 5.0
            curr_k = (ma5 - prev_ma5) / prev_ma5
            last1d_k = (prev_ma5 - last2d_ma5) / last2d_ma5
            last2d_k = (last2d_ma5 - last3d_ma5) / last3d_ma5
            ma5_k = (curr_k - last1d_k) / (last1d_k + 0.00001)
            last_1d_ma5_k = (last1d_k - last2d_k) / (last2d_k + 0.00001)

            #### 最近2次的斜率都>0，&& 持仓天数>=3天（买入当天不算）说明已经启动反攻，若当天的斜率<x时，证明短期不启动则卖出，把仓位留给别只，即未形成多头走势，调仓成其他更可能有溢价的
            if last_1d_ma5_k > 0 and ma5_k > 0 and holding_days >= 3 and ma5_k < 0.5399:
                return True, 6
            ## 止盈,高位离场
            ...
            ## 卖出情况1: 止损位，一天内亏损6.2%
            if cur_time > datetime.time(9, 41) and (last_price - last_1d_close_price) / last_1d_close_price < 0.02:
                return True, 1
            if ma5 < prev_ma5 and cur_time <= datetime.time(9, 41) and (
                    last_price - last_1d_close_price) / last_1d_close_price < -0.01:
                return True, 11
            # if cur_time > datetime.time(9, 41) and (last_price - last_1d_close_price) / last_1d_close_price > 0.02:
    ...
    def should_buy(self, stock_code, current_price, last_1d_close_price, info_dict, is_high_fx, is_noon_time, open_price, volume):
        cur_time = datetime.datetime.now().time()
        ## 当天涨幅>7%说明反转强烈，直接跟进，但可能遇到诱多，先保持观察
        if datetime.time(9, 42) < cur_time < datetime.time(14, 51) and (current_price - last_1d_close_price ) / last_1d_close_price > 0.07:
            return True, 199
        elif cur_time < datetime.time(14, 50) and self.is_test != "1":
            return False, 198
        ...
        # 多头排列&上升趋势
        last_close_str = info_dict.get("last_close", "")
        last_open_str = info_dict.get("last_open", "")
        last_volume_str = info_dict.get("last_volume", "")
        last_close_list = last_close_str.split(",")
        last_open_list = last_open_str.split(",")
        last_volume_list = last_volume_str.split(",")
        if "" == last_close_str or len(last_close_list) < 32:
            return False, 201
        else:
            # 利用最新价格计算当天的MA5，MA10，MA20，MA30
            lc_list = [float(ele) for ele in last_close_list]
            lo_list = [float(ele) for ele in last_open_list]
            lv_list = [float(ele) for ele in last_volume_list]
            ...
            # ma5_rate = (ma5 - prev_ma5) / prev_ma5
            ma5_rate = (ma5 - prev_ma5) / ma5    # 和回测保持一致
            ...
            #increase_status = ma5_rate > 0.0001
            # ma5非最低
            ...
            # return True, 201
            #if increase_status:
            # if current_price > last_1d_close_price:
            ...
            today_close_rate = (current_price - lc_list[-1]) / lc_list[-1]
            #if current_price - open_price >= 0 and current_price > lo_list[-2] and -0.0056 <= today_close_rate < 0.06 \
            #        and (lv_list[-2] < today_volume < lv_list[-1] or lv_list[-2] < volume < lv_list[-1]):

            if (current_price - open_price >= 0 and
                    (volume - lv_list[-1]) / lv_list[-1] < -0.3 and
                    (volume - lv_list[-2]) / lv_list[-2] < -0.3 and
                    (volume - lv_list[-3]) / lv_list[-3] < -0.3 and
                    volume <  lv_list[-1] < lv_list[-2] < lv_list[-3]  and
                    -0.0056 <= today_close_rate < 0.06):
                return True, 202
            else:
                return False, 203
    ...
    async def do(self, accounts):
        print("[DEBUG]do inv ma v52", utils.get_current_time(), accounts)
        target_code = '沪深A股'
        ...
        for has_stock in has_stock_obj:
            ...
            # has_stock_list.append("600817.SH")
            # has_stock_list.append("600337.SH")

            # has_stock_list.append("")
            # has_stock_list.append("")
        ...
        print("subscribe_whole_list:", subscribe_whole_list)
        ...
        for code in has_stock_list:
            ...
            has_open_price = round(has_stock_map[stock].get("open_price", 0), 2)
            code = code.split('.')[0]
            # print("_________has__position_____________", code, has_volume, has_open_price)

            # 只卖出自己策略买入的股票
            if self.get_holding_volume(stock) <= 0:
                continue

            if self.is_today_bought(stock):
                rma_logger.info(f"跳过当天买入股票 {stock} 的卖出检查（T+1限制）")
                continue

            print("_________can__sell_____________", code, has_volume, has_open_price)
            ...
            #         add_volume = 400
            #     elif (latest_price - has_open_price) / has_open_price < -0.06:
            #         add_volume = 300
            #     elif (latest_price - has_open_price) / has_open_price < -0.04:
            #         add_volume = 200
            #     # 加仓条件
            #     order_id = -1
            #     r = dict()
            ...
            #     r['name'] = stock_name
            #     if "1" != self.is_test:
            #         order_id = xt_trader.order_stock(acc, stock, xtconstant.STOCK_BUY, add_volume,
            #                                          xtconstant.FIX_PRICE, latest_price, strategy_name="inv_rma")
            #
            #         # 记录到策略交易记录
            #         self.record_trade(
            #             action='buy',
            ...
            #         )
            #     else:
            #         print('add_volume', json.dumps(r, ensure_ascii=False), 'hold_price:', has_open_price)
            #     rma_logger.info("b:" + json.dumps(r, ensure_ascii=False))
            # else:
            # 卖出条件
            is_sell, sell_id = self.should_sell(stock, last_1d_close_price, max_price, max_price_timestamp, latest_price, bid_price, bid_vol, has_open_price, open_price)
            if is_sell and has_volume > 0:
                ...
                if "1" != self.is_test:
                    ...
                    print(f"[DEBUG]realtime rma is_test=1,sell_stock={stock},sell_price={sell_price}，sell_volume={has_volume}")
        ...
        for rank, (code, score) in enumerate(eff_code_dict.items(), 1):
            ...
            is_buy, buy_id= self.should_buy(stock, latest_price, last_1d_close_price, info_dict, is_high_fx, is_noon_time, open_price, volume)
            print(f"[DEBUG]realtime rma should_buy,is_buy={is_buy},buy_id={buy_id}")
            # rma_logger.info("INFO:" + json.dumps({"stock":stock, "volume":volume}, ensure_ascii=False))
            if not is_buy:
                continue
            ## 均衡仓位
            ...
            if cash >= latest_price * buy_volume:
                ...
                if "1" != self.is_test:
                    order_id = xt_trader.order_stock(acc, stock, xtconstant.STOCK_BUY, buy_volume,
                                                     xtconstant.FIX_PRICE, latest_price, strategy_name="reverse_rma_v5")
                    target_ind += 1
                    ret['order_id'] = order_id
...
if __name__ == "__main__":
    a = ReverseMABULLV5(config="../conf/v1.ini")