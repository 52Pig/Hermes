...
config = {
    ...
    'output_file_prefix': 'inv_ma_bt_52_',
    'ma_windows': [5, 10, 20, 30],
    'min_days': 40,  # 计算均线所需的最小天数
    'history_days': 100,  # 需要加载的历史交易日天数（用于判断涨停等）
    # 'end_date': '20250929'
    'end_date': ed
}
...
def calculate_technical_indicators(stock_data, stock_info):
    ...
    for code, prices in stock_data.items():
        ...
        # odf = pd.DataFrame(prices, columns=['open'])
        ...
        #         latest_price >= 2.0 and latest_price < 36.0 and
        #         last_1d_zf < 0.03 and
        #         last_2d_zf < 0.03 and
        #         last_3d_zf < 0.03 and
        #         last_4d_zf < 0.03 and
        ...
        ## 最近6天跌幅 5%以上
        # last_3d_7d_max_close = df['close'].iloc[i - 9: i - 7].max()
        last_3d_7d_max_close = df['close'].iloc[-8: -6].max()
        is_last_3d_7d_zf_opt = (df['close'].iloc[-2] - last_3d_7d_max_close) / last_3d_7d_max_close
        last_3d_open_zf = df['close'].iloc[-2] - odf['open'].iloc[-2]
        last_3d_zf = (df['close'].iloc[-2] - df['close'].iloc[-3]) /df['close'].iloc[-3]
        ## 当天红柱
        ## 当天涨跌幅范围 [-0.0056, 0.06)
        is_target = (
                     stock_info.get(code, {}).get("circ_mv", 0.0) < 500
                     and last_3d_zf and last_3d_open_zf < 0
                     and is_last_3d_7d_zf_opt < -0.05
                     and (0 <= df['close'].iloc[-1] - odf['open'].iloc[-1])
                     and -0.0056 <= (df['close'].iloc[-1] - df['close'].iloc[-2]) / df['close'].iloc[-2] < 0.06
                     and vdf['volume'].iloc[-1] > vdf['volume'].iloc[-2]
        )

        d = dict()
        d["code"] = code
        d["is_last_3d_7d_zf_opt"] = is_last_3d_7d_zf_opt
        d["close_today"] = df['close'].iloc[-1]
        d["open_today"] = odf['open'].iloc[-1]
        d["close_last1d"] = df['close'].iloc[-2]
        print(json.dumps(d))

        # if code.startswith("600051"):
        #     print(json.dumps(d))
        #     break
        ...
        #     # (last_2d_k < -0.03 and last_1d_k < -0.01)
        #     ( last_1d_k < -0.015
        #     or last_2d_k < -0.03
        #     or last_3d_k < -0.03
        #     or last_4d_k < -0.03
