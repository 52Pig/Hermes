...
config = {
    ...
    'output_dir': 'D:/tool/dataset/strategy_data/',
    'output_file_prefix': 'opt_rtrma_',
            ...
            for line in f:
                ...
                for code, stock_data in data.items():
                    ...
                    if 'd' in stock_data and stock_data['d']:
                        ...
                        open_price = stock_data['d'][1][1]


                        #总成交量
                        volume = sum(minute_data[5] for minute_data in stock_data['d'][:-10])
                        # stock_close_prices[code].append(close_price)
                        stock_close_prices[code].append({
                            'open': open_price,
                            'close': close_price,
                            'volume': volume
                        })
...
def calculate_technical_indicators(stock_data, stock_info):
    ...
    for code, prices in stock_data.items():
        ...
        try:
            ...
            df = df[['close']]
            odf = pd.DataFrame(prices)
            odf = odf[['open']]
            vdf = pd.DataFrame(prices)
            vdf = vdf[['volume']]
            # print(f"df={df}")
            ...
            last_2d_row = valid_df.iloc[-2]
            last_2d_ma5 = last_2d_row['MA5']
            last_3d_row = valid_df.iloc[-3]
            last_3d_ma5 = last_3d_row['MA5']
            last_4d_row = valid_df.iloc[-4]
            last_5d_row = valid_df.iloc[-5]
            last_6d_row = valid_df.iloc[-6]
            last_7d_row = valid_df.iloc[-7]
            last_8d_row = valid_df.iloc[-8]
            last_9d_row = valid_df.iloc[-9]
            last_10d_row = valid_df.iloc[-10]

            last_8d_k = (last_8d_row["MA5"] - last_9d_row["MA5"]) / last_9d_row["MA5"]
            last_7d_k = (last_7d_row["MA5"] - last_8d_row["MA5"]) / last_8d_row["MA5"]
            last_6d_k = (last_6d_row["MA5"] - last_7d_row["MA5"]) / last_7d_row["MA5"]
            last_5d_k = (last_5d_row["MA5"] - last_6d_row["MA5"]) / last_6d_row["MA5"]
            last_4d_k = (last_4d_row["MA5"] - last_5d_row["MA5"]) / last_5d_row["MA5"]
            last_3d_k = (last_3d_row["MA5"] - last_4d_row["MA5"]) / last_4d_row["MA5"]
            last_2d_k = (last_2d_row["MA5"] - last_3d_row["MA5"]) / last_3d_row["MA5"]
            last_1d_k = (last_row["MA5"] - last_2d_row["MA5"]) / last_2d_row["MA5"]
            print(f"last_2d_k={last_2d_k},last_2d_ma5={last_2d_ma5},last_3d_ma5={last_3d_ma5}")
            print(f"last_1d_k={last_2d_k},last_ma5={last_row['MA5']},last_2d_ma5={last_2d_ma5}")
            ...
            is_not_up_limit_before_5day = (up_limit_days == 0)
            ...
            is_t1 = (
                    #last_2d_zf < 0.04 and last_1d_zf < 0.04 and  # 昨天涨幅6%以下，\
                    odf['open'].iloc[-1] < df['close'].iloc[-1] and   ## 如果当天是阴线，错过介入时机
                    last_1d_k > 0 and                                 ## 当天趋势正向
                    is_not_up_limit_before_5day and
                    last_2d_k > -0.01 and                             ## 昨天开始正向反转
                    (
                            last_3d_k < -0.015  # 昨天以前必须都是跌
                            and last_4d_k < 0  #
                            and last_5d_k < 0
                            and last_6d_k < 0
                    ) and (
                            last_4d_k < -0.03  # 3天前巨大跌幅
                            or last_5d_k < -0.03  # 4天前巨大跌幅
                            or last_6d_k < -0.03  # 5天前巨大跌幅
                            or last_7d_k < -0.03  # 6天前巨大跌幅
                            or last_8d_k < -0.03  # 7天前巨大跌幅
                    )
            )

            is_t3 = (
                stock_info.get(code, {}).get("circ_mv", 0.0) < 490 and
                last_1d_zf < 0.06 and last_2d_zf < 0.06 and  # 昨天涨幅6%以下，
                vdf['volume'].iloc[-1] < vdf['volume'].iloc[-2] < vdf['volume'].iloc[-3] and
                    is_not_up_limit_before_5day and
                    (
                            last_1d_k < -0.015  # 昨天以前必须都是跌
                            and last_1d_k < 0  #
                            and last_2d_k < 0
                            and last_3d_k < 0
                    ) and (
                            last_1d_k < -0.015  # 3天前巨大跌幅
                            or last_2d_k < -0.03  # 4天前巨大跌幅
                            or last_3d_k < -0.03  # 5天前巨大跌幅
                            or last_4d_k < -0.03  # 6天前巨大跌幅
                            or last_5d_k < -0.03  # 7天前巨大跌幅
                            or last_6d_k < -0.03  # 7天前巨大跌幅
                    )
            )
            ...
            results[code] = {
                **stock_info.get(code, {}),
                ...
                'last_volume': ','.join(map(str, vdf['volume'].tail(100).tolist())),
                'last_open': ','.join(map(str, odf['open'].tail(100).tolist())),
                'MA5': ma5,
                'MA10': ma10,
                'MA20': ma20,
                'MA30': ma30,
