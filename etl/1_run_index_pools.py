...
def gen_stock_pools():
    ...
    sz_index_codes = xtdata.get_stock_list_in_sector('深市指数')
    print(sz_index_codes)

    # 获取沪市指数板块下的所有指数代码
    sh_index_codes = xtdata.get_stock_list_in_sector('沪市指数')
    index_stocks = sh_index_codes + sz_index_codes
    ## 筛选出有效准入池
    recall_stock = list()
    stc_dict = dict()
    ...
    if len(stc_dict) > 10:
        ...
        stc_json = open(os.path.join(data_path, 'index_pools.json'), 'w')
        stc_str = json.dumps(stc_dict, ensure_ascii=False, indent=4)
        stc_json.write(stc_str+'\n')

        meta = open(os.path.join(data_path, 'index_pools.meta'), 'w')
